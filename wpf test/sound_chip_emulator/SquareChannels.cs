...
{
    public class Square1 : ISampleProvider
    {
        private const int envelope_clock_freq = 64;
        private const int sweep_clock_freq = 128;
        private enum SweepDirection : int
        {
            ADD = 0,
            SUB = 1
        }
        private enum VolumeEnvelopeDirection : int
        {
            OFF = 0,
            UP = 1,
            DOWN = -1
        ...
        public Square1(SquareRegisters s)
        {
            WaveFormat = WaveFormat.CreateIeeeFloatWaveFormat(16000, 1);
            setFromRegister(s);
        }
        public void setFromRegister(SquareRegisters s)
        {
            onsample = 0;
        ...
        private float getNextSample()
        {
            ...
            while (phase >= 8)
            { // while loop not if for high frequencies, might overshoot table with just 1 subtraction of table length
                phase -= 8;
            }
            int index = (int)phase;
            ...
            return tables[(int)waveform, index] * gain;
